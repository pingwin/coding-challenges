...
bubble_sort_single_pass(Direction, [H,N|Tail], [], Changed) ->
	case compare(Direction, H, N) of
		true ->
			bubble_sort_single_pass(Direction, Tail, [H, N], Changed);
		false ->
			bubble_sort_single_pass(Direction, Tail, [N, H], true)
	end;
...
bubble_sort_single_pass(Direction, [H|Tail], Acc, Changed) ->
	...
	case compare(Direction, Last, H) of
		true ->
			bubble_sort_single_pass(Direction, Tail, Acc ++ [H], Changed);
		false ->
			bubble_sort_single_pass(Direction, Tail, lists:droplast(Acc) ++ [H, Last], true)
	end.
...
bubble_sort(Direction, List) ->
	case bubble_sort_single_pass(Direction, List, [], false) of
		{PartialSorted, true} -> bubble_sort(Direction, PartialSorted);
		{Sorted, false} -> Sorted
	end.
